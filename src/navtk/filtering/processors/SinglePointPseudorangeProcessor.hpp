...
/**
 * Updates a Pinson15NedBlock, a ClockBiasesStateBlock, and a processor-managed collection of
 * FogmBlock instances using measurements provided as filtering::PairedPva (wrapping a
 * MeasurementSatnav) and associated GpsEphemerisAux. Each PRN's pseudorange bias is estimated using
 * a First-order Gaussian Markov (FOGM) state block. The FOGM state blocks are automatically
 * created/removed from inside SinglePointPseudorangeProcessor based on the PRNs available in the
 * pseudorange measurement. Each PRN's pseudorange bias state block label is a concatenation of
 * #pr_bias_sb_label + aspn_xtensor::TypeSatnavObs::get_prn(). As an example label, PRN 32 would
 * have the pseudorange bias state block label `"pr_bias_sv_32"`. This processor corrects
 * pseudoranges for ionospheric delay (if the receiver is dual frequency), group delay (when
 * applicable), satellite clock error, and tropospheric delay.
 */
class SinglePointPseudorangeProcessor : public MeasurementProcessor<> {

public:
	/**
	 * Main constructor for the processor.
	 * @param label The identifier for this instance of this processor.
	 * @param pinson15_label The label to an instance of Pinson15NedBlock.
	 ...
	 */
	SinglePointPseudorangeProcessor(std::string label,
	                                std::string pinson15_label,
	 ...
	 * If clock initialization was not forced (`force_clock_initialization` parameter is `false` in
	 * constructor call) then the return from the first call to this function will assume that
...
protected:
	 ...
	 * @param corrected_meas Return value of correct_measurements()
	 */
	 ...
	 * is Pinson15NedBlock error states, receiver clock bias states, and pseudorange bias states.
	 *
	 ...
	 *
	 * @param meas Return value of correct_measurements()
	 * @param ecef_pos Error state corrected sensor ECEF position in meters
	 * @param xhat State Vector consisting of at least 15 Pinson states (as in Pinson15NedBlock,
	 * and with position error states corresponding to #XHAT_POS_INDICES) and 2 clock bias states
	 * (the first corresponding to #clock_state_index).
	 *
	...
	/**
	 * Build a measurement model that updates NED position error states, clock bias and drift, and
	 * pseudorange biases.
	 *
	 * @param meas Return value of correct_measurements()
	 * @param ecef_pos Error state corrected sensor ECEF position in meters
	 * @param H Measurement function Jacobian mapping Pinson error states, clock bias states and
	 * pseudorange bias states to a Vector of pseudorange measurements
	 *
	 ...
	 * @param ecef_pos Error state corrected sensor ECEF position in meters
	 *
	 * @return N predicted measurements where \f$ h(x) = \hat{r}_{sv} + \hat{bias}_{sv} + C *
	 * bias_{rcvr} \f$
	 */
	Vector generate_h(const Vector &xhat, const Matrix &sv_pos, const Vector3 &ecef_pos) const;

	/**
	 * Estimates SV (Space Vehicle) clock error, position, elevation, and azimuth. Corrects
	 * pseudorange measurements for group delay, clock error, tropospheric delay.
	 *
	 ...
	 */
	CorrectedGnssPseudorangeMeasurement correct_measurements(
	...
	/**
	 * Creates a FOGM state block for a single PRN pseudorange bias estimate. Adds the state
	 * block to the SinglePointPseudorangeProcessor.
	 *
	...
	/**
	 * Specifies what frequency and code to select a pseudorange from.
	 */
	gnssutils::PseudorangeType pr_to_use;

	/**
	 * Determines the covariance generated by the MeasurementProcessor.
	 */
	double pr_noise_covariance;
	...
	/**
	 * The index of the clock bias state, assumed to be 16th state
	 */
	Size clock_state_index = 15;

	/**
	 * Indices of position elements within the estimate (`xhat`)
	 */
	static constexpr std::pair<Size, Size> XHAT_POS_INDICES = std::make_pair(0, 3);

	/**
	 * The beginning index of the pseudorange bias states, assumed to be 18th state.
	 * The end index determined based on the pseudorange measurements observed, temporarily
	 * set to 0.
	 */
	std::pair<Size, Size> xhat_pr_bias_index = std::make_pair(17, 0);

	/**
	 * Label for generating the pseudorange bias FOGM state blocks. The labels
	 * for each PRN will be in the format `"pr_bias_sv"`+`PRN_NUM`. For example, PRN 32
	 * will have the PRN bias state block label `"pr_bias_sv_32"`.
	 */
	const std::string pr_bias_sb_label = "pr_bias_sv_";

private:
	/*
	 * Generates the line of sight vectors need to relate Pinson15NedBlock position error states
	 * to an estimated pseudorange (C_ecef_to_ned * los_ecef, per SV (Space Vehicle)).
	 *
	 * @param ranges RangeInfo for tracked SVs.
	 * @param ecef_pos ECEF position coordinates of receiver.
	 */
	std::vector<Vector3> calculate_los(const std::vector<RangeInfo> &ranges,
