...
/**
 * Processes Measurements containing a MeasurementDeltaPosition and NavSolution to update velocity
 * error states.
 */
class DeltaPositionMeasurementProcessor : public MeasurementProcessor<> {
...
public:
	 ...
	 * @param use_term1 When true, attempts to use the term1 component of the measurement. Throws an
	 * error if this component is null (NaN).
	 * @param use_term2 When true, attempts to use the term2 component of the measurement. Throws an
	 * error if this component is null (NaN).
	 * @param use_term3 When true, attempts to use the term3 component of the measurement. Throws an
	 * error if this component is null (NaN).
	...
	/**
	 * Generates a StandardMeasurementModel that relates a PairedPva (wrapping a
	 * MeasurementDeltaPosition) to a state vector with NED velocity error states.
	 *
	 * @param measurement Measurement of type MeasurementDeltaPosition. The x, y, and z fields of
	 * the MeasurementDeltaPosition should respectively contain delta North, East, and Down
	 * estimates in meters.
	 *
	 ...
	 * MeasurementDeltaPosition, otherwise `nullptr`.
	 */
...
private:
	...
	Size index2;
	Size index3;
	const bool use_term1;
	const bool use_term2;
	const bool use_term3;
	const AspnMeasurementDeltaPositionReferenceFrame expected_frame;
