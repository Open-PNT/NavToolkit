...
/**
 * Shifts an ECEF representation (with quaternion attitude) of the platform frame origin to a
 * sensor frame. Velocity is not affected (any tangential velocity effects as a result of the
 * platform rotating are not accounted for in the shift). Output quaternion is sensor to ECEF, in
 * contrast with the input platform to ECEF attitude.
 *
 ...
 *
 * 7 - 'Own' to ECEF quaternion 'i' vector element. (Own is platform for input, sensor for output).
 *
 ...
 */
class PlatformToSensorEcefQuat : public VirtualStateBlock {
public:
	/**
	 * Constructor.
	 ...
	 */
	PlatformToSensorEcefQuat(std::string current,
	                         std::string target,
