...
/**
 * Shifts an ECEF representation of a sensor frame origin to the platform frame. Velocity is not
 * affected (any tangential velocity effects as a result of the platform rotating are not accounted
 * for in the shift). Output angles reflect the ECEF to platform orientation, in contrast with the
 * input ECEF to sensor attitude.
 *
 * Inputs/output state units:
 *
 ...
 */
class SensorToPlatformEcef : public VirtualStateBlock {
public:
	/**
	 * Constructor.
	 *
	 ...
	 * @param scale_factor Scale factor applied to ECEF positions by to alleviate some numerical
	 * issues. A value of 1000 results in km position representation. Output position units are not
	 * altered.
	 */
	SensorToPlatformEcef(std::string current,
	                     std::string target,
