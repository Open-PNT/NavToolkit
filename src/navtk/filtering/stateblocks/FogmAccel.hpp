...
/**
 * Models a set of position, velocity, and acceleration states in 3 dimensions, where the dynamics
 * of the acceleration states are first-order Gauss-Markov (FOGM) processes.
 */
class FogmAccel : public FogmBlock {
public:
	/**
	 * @param label The label uniquely identifying this particular set of states.
	 * @param time_constants The FOGM time constants (decay term) for each accel state in seconds.
	 * @param state_sigmas The 1-sigma value of each accel state at steady-state where the first
	 * three values are in meters, the next three are in m/s, and the last are m/s^2.
	 * @param num_dimensions The number of dimensions of space needed. For example, if
	 * `num_dimensions=2`, then this block will have 6 states: X and Y position, X and Y velocity,
	 * and X and Y acceleration.
	 ...
	 */
	FogmAccel(const std::string& label,
	...
	not_null<std::shared_ptr<StateBlock<>>> clone() override {
		return std::make_shared<FogmAccel>(*this);
	}
...
protected:
	 ...
	 */
	void populate_f_and_q(Matrix& F, Matrix& Q) override;
};
