...
def list_projects():
    ...
    entries = sorted([e for e in entries if e not in private_repos])
    for i in entries:
        print(i)
    return 0
...
def unpack(sproj, branch):
    ...
    subprocess.check_call(
        [
            ...
            '-b',
            branch,
            'https://github.com/mesonbuild/{}.git'.format(sproj),
            tmpdir,
        ]
    )
    ...
    if os.path.isdir(outdir):
        print(
            f'Subproject is already there. To update, nuke the {outdir} dir '
            + 'and reinstall.'
        )
...
def install(sproj, requested_branch=None):
    if not os.path.isdir(spdir):
        print(
            'Run this in your source root and make sure there is a '
            + 'subprojects directory in it.'
        )
        return 1
    blist = gh_get(
        'https://api.github.com/repos/mesonbuild/{}/branches'.format(sproj)
    )
    blist = [b['name'] for b in blist]
    blist = sorted([b for b in blist if b != 'master'])
    branch = blist[-1]
...
if __name__ == '__main__':
    print(
        'This is an emergency wrap downloader. Use only when wrapdb is down.'
    )
